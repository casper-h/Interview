...
The material in `system_design/plan/` builds deep knowledge. This document is different: it is
...
1. **Clarify requirements (roughly 3 to 5 minutes).** Identify the functional requirements: what
   does the system actually need to do? List the core use cases. Identify the non-functional
   requirements: scale (users, queries per second, data volume), latency targets, availability
   target, consistency needs, and the read/write ratio. Explicitly scope out what you are not
   designing, and say so out loud; this is a senior signal, since it shows you can bound a
   problem.
...
6. **Deep dive (roughly 10 to 15 minutes), usually interviewer-directed.** Pick one or two
   ...
   the content covered in `system_design/plan/` actually get used. Follow the interviewer's
   signal; if they lean into one area, that is where depth is expected.
...
8. **Wrap-up (roughly 2 to 3 minutes).** Cover monitoring and observability, what you would do
   with more time, and what you would reconsider at ten times the scale.
...
- SSD random read: roughly 100 microseconds.
...
- **Infrastructure or platform teams** tend to probe more deeply into the storage engine,
  consensus, and replication internals covered in `system_design/plan/`, and the optional
